...
extern "C" __global__
void t2dPDE_center_clamp_step(int ncols, int nrows, float alpha, float* temp_in, float* temp_out)
{
...
extern "C" __global__
void t2dPDE_center_clamp(int threads, int blocks,
    int nsteps, int ncols, int nrows, float alpha, float* temp_in, float* temp_out)
{
    ...
    {
        t2dPDE_center_clamp_step << <gridSize, blockSize >> > (ncols, nrows, alpha, left, right);
