...
namespace NcuBatch
{
    class Printer
    {
        private IEnumerable<string> kernels = new List<string>() { "transpose", "sgemm", "dgemm" };
        ...
        public void Print()
        {
            foreach (var kernel in kernels)
            {
                ...
                {
                    var param = $"-k {kernel} -b {size.Item1} -t {size.Item2} -r {ipow2(12)}";
                    var output = $"{kernel}-{size.Item1}-{size.Item2}-{ipow2(12)}.txt";
                    Console.WriteLine(string.Format(ncuTemplate, kernel, output) + param + System.Environment.NewLine);
                }
            }
        }
    }
