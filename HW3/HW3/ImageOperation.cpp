...
{
	// we need to read the file to know the image size so allocation is done in InitData
}
...
void ImageOperation::InitData()
{
	...
	{
		h_input_float_image[i] = (int) h_input_char_image;
	}
...
void ImageOperation::VerifyResult()
{
	...
	for (size_t i = 0; i < size; i++)
	{
		h_output_char_image[i] = (unsigned char)(unsigned int)h_output_float_image;
	}
